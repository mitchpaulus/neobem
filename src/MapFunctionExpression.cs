...
namespace src
{
    public class MapFunctionExpression : FunctionExpression
    {
        ...
        public override (string, Expression) Evaluate(List<Expression> inputs, string baseDirectory)
        {
            ...
            {
                throw new ArgumentException(
                    $"Expected a list as the first parameter to map, received a {inputs[0].TypeName()} of value '{inputs[1].AsErrorString()}'");
            }

            if (inputs[1] is not FunctionExpression function)
            {
                throw new ArgumentException(
                    $"Expected a function as the second parameter to map, received a {inputs[1].TypeName()} of value '{inputs[0].AsErrorString()}'");
            }
